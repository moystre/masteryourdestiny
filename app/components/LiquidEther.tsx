...
export default function LiquidEther({
    ...
    useEffect(() => {
        ...
        class AutoDriver {
            ...
            update() {
                ...
                const step = this.speed * dtSec * ramp, move = Math.min(step, dist)
        ...
        class ExternalForce extends ShaderPass {
            ...
            update(...args: any[]) {
                const props = args[0] || {}
                const forceX = (Mouse.diff.x/2)*(props.mouse_force||0), forceY = (Mouse.diff.y/2)*(props.mouse_force||0)
                const cs = props.cellScale||{x:1,y:1}, cursorSize = props.cursor_size||0
                const csx = cursorSize*cs.x, csy = cursorSize*cs.y
                const cx = Math.min(Math.max(Mouse.coords.x,-1+csx+cs.x*2),1-csx-cs.x*2)
                const cy = Math.min(Math.max(Mouse.coords.y,-1+csy+cs.y*2),1-csy-cs.y*2)
                const u = (this.mouse.material as THREE.RawShaderMaterial).uniforms
                u.force.value.set(forceX,forceY); u.center.value.set(cx,cy); u.scale.value.set(cursorSize,cursorSize)
                super.update()
            }
        }
        class Viscous extends ShaderPass {
            ...
            update(...args: any[]) {
                ...
                for (let i = 0; i < (iterations??0); i++) {
                    ...
                    if (typeof dt==="number") this.uniforms.dt.value=dt; super.update()
                }
                return fbo_out
            }
        }
        ...
        class Poisson extends ShaderPass {
            ...
            update(...args: any[]) {
                ...
                for (let i = 0; i < (iterations??0); i++) {
                    ...
                    if (this.uniforms) this.uniforms.pressure.value=p_in.texture; this.props.output=p_out; super.update()
                }
                return p_out
            }
        }
        class Pressure extends ShaderPass {
            ...
            update(...args: any[]) { const { vel, pressure } = (args[0]||{}) as any; if (this.uniforms&&vel&&pressure) { this.uniforms.velocity.value=vel.texture; this.uniforms.pressure.value=pressure.texture } super.update() }
        }
        class Simulation {
            options: SimOptions
            ...
            createAllFBO() {
                const type=this.getFloatType(), opts={ type, depthBuffer:false, stencilBuffer:false, minFilter:THREE.LinearFilter, magFilter:THREE.LinearFilter, wrapS:THREE.ClampToEdgeWrapping, wrapT:THREE.ClampToEdgeWrapping } as const
            ...
            calcSize() {
                ...
                const cw=Math.min(w,512), ch=Math.min(h,512)
        ...
        class WebGLManager implements LiquidEtherWebGL {
            ...
            dispose() {
                try { window.removeEventListener("resize",this._resize); if(this._onVisibility) document.removeEventListener("visibilitychange",this._onVisibility); Mouse.dispose(); if(Common.renderer){const canvas=Common.renderer.domElement;if(canvas&&canvas.parentNode)canvas.parentNode.removeChild(canvas);Common.renderer.dispose()} } catch { /* noop */ }
            }
        }
        ...
        const io = new IntersectionObserver((entries) => {
            const entry=entries[0], isVis=entry.isIntersecting&&entry.intersectionRatio>0
        ...
        ro.observe(container,{box:"content-box" as ResizeObserverBoxOptions}); resizeObserverRef.current=ro
        return () => {
    ...
    return (
        <div ref={mountRef} className={`liquid-ether-container ${className||""}`} style={style}>
            <style>{`.liquid-ether-container{position:relative;overflow:hidden;width:100%;height:100%;touch-action:none;}`}</style>
        </div>
    )
}
